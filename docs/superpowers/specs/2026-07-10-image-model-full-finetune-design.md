...
The shared engine owns only the lifecycle that differs from LoRA training: making the transformer trainable, building
the optimizer over transformer parameters, preparing that transformer with Accelerate, and saving full checkpoints.
The thin entrypoints inherit the existing `*NetworkTrainer` classes to reuse model-specific hooks. Existing LoRA and
existing full-finetune scripts stay unchanged. This is the selected approach because it removes meaningful duplication
without widening the regression surface to every trainer.
...
The mixin also defines two narrow full-finetune hooks:
...
methods such as `on_step_start`, `save_weights`, or max-norm regularization.

### Training lifecycle

The engine performs these steps in order:

1. Validate common and model-specific arguments before loading the checkpoint.
2. Set CUDA TF32/cuDNN flags, resolve the seed, and build the cached training dataset using the model architecture key.
3. Prepare Accelerate and resolve the trainable parameter dtype:
...
5. Load a plain, non-quantized DiT, run `on_transformer_loaded`, cast every floating trainable parameter to the selected
   trainable dtype, and set all intended DiT parameters to `requires_grad=True`. The post-load cast is mandatory because
   some existing network loaders, notably FLUX.1 Kontext, do not honor their `dit_weight_dtype` argument today.
6. Enable ordinary backward-capable block swap and gradient checkpointing when requested.
7. Build one optimizer parameter group from named trainable transformer parameters and fail if the group is empty.
8. Prepare the transformer, optimizer, dataloader, and scheduler with Accelerate. The transformer itself is the
   accumulation/training model.
9. For every batch, reuse `scale_shift_latents` and `process_batch`, backpropagate the returned scalar loss, clip the
   transformer gradients, step the optimizer/scheduler, and log the shared and model-specific loss metrics.
10. Preserve current step, epoch, sampling, state-save, retention, tracker, and Hugging Face upload behavior.
11. Save the unwrapped transformer at step/epoch boundaries and at the end of training.
...
The script must reject incompatible optimizers or process counts before the first backward pass.
...
The resolved save dtype follows `--save_precision` when explicitly set. Otherwise it matches the trainable parameter
dtype: fp32 by default and bf16 with `--full_bf16`. Casting to a different explicit save precision may require
additional memory and must be called out in the documentation. `--mem_eff_save` uses the repository's streaming
safetensors writer; Accelerate state saving remains separate and may still require normal optimizer-state memory.
...
Accelerate state save/resume includes the full prepared transformer and optimizer. A normal exported safetensors
checkpoint is a model checkpoint, not a replacement for `--save_state` when exact optimizer/scheduler resume is needed.
...
- reject unsupported fused-backward optimizers and multi-process fused-backward launches;
- reject an explicit save precision that the safetensors writer cannot represent.
...
- Keep `--fp8_text_encoder` available only for the frozen sample-prompt encoder and retain its existing model-version
  restrictions.
- Store the selected model version in checkpoint metadata so users do not have to infer it from the filename.
...
The small Ideogram sampling-policy seam may be generalized so the existing LoRA trainer preserves its opt-in behavior
while the full trainer opts in whenever `--unconditional_dit` is supplied. This refactor must not change LoRA defaults.
...
- Sample from the current trainable checkpoint. A Turbo checkpoint may be finetuned only by passing it directly as
  `--dit`, in which case it is the training model rather than a temporary sampling model.
...
Small existing-file changes are allowed only where needed to expose a behavior-neutral hook, most notably the Ideogram
4 unconditional sampling policy. The existing Qwen-Image, Z-Image, HiDream-O1, and all LoRA entrypoints remain on their
...
- All distributed saves wait for every process; only the main process writes exported model files.
...
5. LoRA arguments, FP8 base training, H2D-only block swap, and incompatible fused backward fail before the loader runs.
6. The ordinary block-swap path requests backward support.
...
11. The prepared-transformer path relies on Accelerate gradient synchronization and performs no second manual reduce.
...
- Monkeypatch each loader to verify the requested trainable dtype and that all transformer parameters become trainable.
...
- Verify Ideogram rejects BNB 4-bit/FP8 headers, preserves `model_type=ideogram4_cond`, and automatically uses a supplied
  frozen unconditional DiT only for sampling.
- Verify Krea 2 rejects Turbo swapping but accepts a Turbo file when it is the primary `--dit` path.
...
4. Ordinary backward-capable block swap, gradient checkpointing, sampling, Accelerate state resume, and full checkpoint
   export are wired through existing model behavior.
