...
class Automagic3(torch.optim.Optimizer):
    ...
    def _make_accum_hook(self):
        ...
        def _hook(p: torch.Tensor):
            if p.grad is None:
                return
            target_device = torch.device("cpu") if self.offload_gradients else p.grad.device
            if hasattr(p, "_accum_grad"):
                acc = p._accum_grad.to(torch.float32).add_(
                    p.grad.to(device=target_device, dtype=torch.float32)
                )
                self._stochastic_copy_(p._accum_grad, acc)
            else:
                p._accum_grad = p.grad.detach().to(device=target_device, copy=True)
            p.grad = None
    ...
    def get_learning_rates(self) -> List[float]:
        ...
        out = []
        for group in self.param_groups:
            # Every parameter mirrors the same group rate. Reading one avoids
            # stacking state tensors that block swapping may leave on mixed
            # CPU/CUDA devices in non-fused fallback mode.
            lr = next(
                (
                    self.state[p]["lr"]
                    for p in group["params"]
            ...
            out.append(float(lr.detach()) if isinstance(lr, torch.Tensor) else float(lr))
        return out
