...
def load_krea2_dit(
    ...
    if quantized or has_lora:
        ...
        if fp8_scaled:
            ...
            # INFO-level logging is suppressed in the GUI app, so echo the active base-weight
            ...
            print(
                f"Krea 2: base weights quantized to ConvRot INT8 "
                f"({sum(1 for k in sd if k.endswith('.scale_weight'))} Linears, "
                f"backward={convrot_int8_bwd}, "
                + ("fused Triton INT8 GEMM" if _HAS_TRITON else "NO TRITON -> slow dequantized BF16 fallback")
                + ")"
            )
