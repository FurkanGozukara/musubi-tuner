					...
					<FormGroup title="Quantization">
						<div class="space-y-2 pt-2">
							...
							<div class="grid grid-cols-3 gap-x-4 gap-y-1">
								...
								<FormToggle label="W4A4G8" fieldPath="training.w4a4g8" checked={t.w4a4g8 ?? false} onchange={(e) => update('w4a4g8', e.target.checked)} tooltip="4-bit weights/activations (the W4A4G4 forward) with 8-bit gradients (the W4A8 backward): the stability middle rung between W4A4G4 and W4A8. int4 container only. Mutually exclusive with W4A4G4, W4A8 and FP8/NF4/INT8 base modes." />
								<FormToggle label="INT4 No MSE Clip" fieldPath="training.int4_convrot_no_mse_clip" checked={t.int4_convrot_no_mse_clip ?? false} onchange={(e) => update('int4_convrot_no_mse_clip', e.target.checked)} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8)} tooltip="Use plain absmax row scales instead of MSE-optimal clipping during on-the-fly INT4 ConvRot quantization." />
							</div>
							<div class="grid grid-cols-3 gap-2">
