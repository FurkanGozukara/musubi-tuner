		...
		<FormGroup title="LTX-2 Performance">
			<div class="space-y-2 pt-2">
				<div class="grid grid-cols-2 gap-x-4 gap-y-1">
					...
					<FormToggle label="Async backward block prefetch" fieldPath="full_finetune.ltx2_block_swap_async_backward" checked={t.ltx2_block_swap_async_backward ?? false} onchange={(e) => update('ltx2_block_swap_async_backward', e.target.checked)} disabled={!t.gradient_checkpointing || !t.use_pinned_memory_for_block_swap || !(t.blocks_to_swap > 0)} tooltip="Overlap the next pinned block load with backward compute; measured about 4% faster for 512×512×33 full fine-tuning." />
